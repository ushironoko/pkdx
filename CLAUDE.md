...
- **nash**: 零和ナッシュ均衡ソルバー / 選出最適化（`pkdx select`） / メタ乖離分析（`pkdx meta-divergence`）。`pkdx select` は回復技（じこさいせい等）・ドレイン技（ギガドレイン等）・やどりぎのタネ・ねむる・いたみわけ・ねがいごと・天候依存回復（つきのひかり等）を考慮する。天候設定技を撃った後に天候依存回復が強化される動的挙動や、ヘドロえき / ふみん / マジックガード等の関連特性による回復技・状態異常の干渉もモデル化される。
...
1. ユーザーから要望や質問なしに、エージェントがレスポンスに独自の解釈の結果を含めてはならない。事実（DB クエリ結果・計算結果）のみを返し、評価・推奨・相性判断などの主観的解釈はユーザーから明示的に求められた場合にのみ提供する。
    - 例（NG）: 〇〇は✕✕に有利なため相性が良い
    - 例（NG）: 〇〇なので✕✕すべきです
...
6. 性別でステータス・タイプ・特性が異なるポケモン (イダイトウ / イエッサン / パフュートン / ニャオニクス 等) を扱う際は、性別を明示した名前で `pkdx query` する。受け入れ可能な表記:

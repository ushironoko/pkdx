...
スキーマ定義は `src/writer/schema.mbt` の `pokemon_schema()` がSSoT。初期JSONは以下のコマンドで生成する:
...
## Stat Card / Training State の表示ルール

Phase 1-7 の各 Stat Card / Training State テンプレは **version で表示列を切り替える**。skill 本文のテンプレは旧版（scarlet_violet 等）の体裁で書かれているため、Champions では下記マッピングで読み替える。

### 列マッピング

| skill 本文の列 | Champions（version=champions） | 旧版（scarlet_violet 等） |
|---|---|---|
| 個体値列 | **非表示**（SP に統合済み、IV は存在しない） | `31`（IV 固定） |
| 努力値列 | **`SP` 列に読み替え**（各 0-32） | `EV` 列（各 0-252） |
| 合計表示 | `<total>/66 (残り: <remaining>/66)` | `<total>/510 (残り: <remaining>/510)` |

### Phase 6-1 推奨配分提案の version 別テンプレ

skill 本文は EV 制（H4 A252 S252 等）で書かれているため、Champions では下記に置き換える。

**Champions（合計 ≤ 66、各 ≤ 32）**:

- 物理アタッカー型: `ASぶっぱ: H4 A32 S30` / `HAベース: H32 A32 S2` / `耐久調整: H32 A2 B32`
- 特殊アタッカー型: `CSぶっぱ: H4 C32 S30` / `HCベース: H32 C32 D2` / `耐久調整: H32 C2 D32`
- 耐久型: `HBベース: H32 B32 D2` / `HDベース: H32 B2 D32`

**旧版（合計 ≤ 510、各 ≤ 252）**: skill 本文の Phase 6-1 表記そのまま（H4 A252 S252 等）
...
|---|------|--------|-----------|-------------|
| 1 | ゲームバージョンは？ | バージョン | champions(default / 推奨, SP 制), scarlet_violet(deprecated, 旧 EV/IV 制), legendsza(deprecated), Other(バージョン名を入力) | false |

**推奨の理由**: Champions 以降は SP (Stat Points) 制に一本化されており、実数値算出・ダメ計・hbd 最適化ともに SP 前提で最適化されている。旧 EV/IV 制 (`scarlet_violet` / `legendsza` 等) は後方互換のために残しているが、新規育成は `champions` を選択すること。
...
### learnset 外の技を Other 入力された場合

`pkdx moves` は learnset 内の技のみを返すため、Other で learnset 外の技（例: SV では非習得・廃止された技）を指定された場合の取扱:

1. cache の `moves[].name` に名前のみ格納する
2. `type` / `category` / `power` / `accuracy` は **`null`** で保留する
3. Phase 7 のダメ計（`pkdx damage --format json`）が出力 JSON に技情報を含むため、ダメ計実行時に値を補填する
4. 攻撃技の場合は最低 1 回 Phase 7 のダメ計を通すよう案内する（補填しないと Phase 8 の `pkdx write pokemon` が `null` を弾く）
5. 変化技の場合はユーザーに「ダメ計対象外なので type/category 情報なしで保存します」と案内し、cache の null のまま Phase 8 へ進める（writer 側で許容）

learnset 外の技は将来 patch 必須になる可能性があるため、可能な限り learnset 内の技を選ぶよう推奨する。

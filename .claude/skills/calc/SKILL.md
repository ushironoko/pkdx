...
| # | 質問 | header | オプション |
|---|------|--------|-----------|
| 4 | 味方のひんし数は？ | ひんし数 | 1, 2(default), 3, 4, 5 |
...
- **防御側**: HP最大投資 + 該当防御ステータス最大投資 + 性格補正↑（物理技→HB特化、特殊技→HD特化）
...
### その他
テラスタル（`--tera-type`）, 急所（`--critical`）, ランク補正（`--atk-rank`, `--def-rank`、-6〜+6、第3世代以降仕様。急所時は攻撃側の負ランク・防御側の正ランクを無視。`--atk-stat`/`--def-stat` 併用時は override 優先）, ひんし数（`--fainted-count`、0-5、そうだいしょう用）

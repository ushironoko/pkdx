...
- **性別でステータス・タイプ・特性が異なる種** (イダイトウ / イエッサン / パフュートン / ニャオニクス) は性別を明示する: jpn `イダイトウ（オス）` / `イダイトウ（メス）`、eng `Basculegion (Male)` / `Basculegion (Female)`。性別未指定の `イダイトウ` は M base のステータスを返すため、F 個体を計算する際は必ず `（メス）` 付きで照合する (例: イダイトウ♂ atk 112 / spa 80、♀ atk 92 / spa 100 とダメージ計算結果が大きく変わる)。ユーザー入力が `イダイトウ♂` / `イダイトウ♀` (ゲーム内表記) で来た場合は `♂ → （オス）` / `♀ → （メス）` に正規化してから query する
...
| 3 | 攻撃側の実数値は？ | 攻撃数値 | デフォルト(SP=32 **最大投資**+性格で自動計算), Other(数値入力) |
| 4 | 防御側の実数値は？ | 防御数値 | デフォルト(SP=0 **無投資**+性格で自動計算), Other(数値入力) |
| 5 | 天候は？ | 天候 | なし(default), はれ, あめ, すなあらし |

CLI のデフォルト実数値は **攻防非対称** (攻撃側 SP=32/EV=252 + 性格 1.1、防御側 SP=0/EV=0 無補正)。「指定なし = 無投資」では **ない** ので、ユーザーの個体がデフォルトと異なる場合は必ず実数値を逆算して `--atk-stat` / `--def-stat` / `--def-hp` で渡す。詳細は `references/special_cases.md` Section 0。
...
| `--protean-consumed` | へんげんじざい/リベロが既に発動済み | bool flag | 既定はその交代出しで初発動と見なし技タイプに STAB を付与。本フラグ指定時は発動済み(2手目以降)として Protean/Libero 由来 STAB を付与しない。変化後タイプと一致する技は攻撃側タイプにその実タイプを渡して通常 STAB で計算する。詳細は `references/special_cases.md` 参照 |
...
**タイプ相性の出力規律 (CRITICAL — 違反は Stop hook `type-claim-guard` がブロックする)**:
- 「タイプ相性」欄の倍率は **必ず `pkdx type-chart <技タイプ> <防御タイプ(,区切り)>` を実行し、その出力倍率をそのまま転記**する。技タイプは `input.move.type`、防御タイプは `input.defender.types` を使う。記憶や暗算で倍率を書いてはならない。
- type-chart の出力倍率は最終集合 (例: ほのお→はがね/ひこう は `2x`)。**個別タイプの倍率を散文で分解して語ってはならない** (「鋼に4倍だが飛行が等倍で複合2倍」のような説明は捏造の温床。type-chart は既に複合済みの値を返す)。
- 出力は **下記テンプレートのテーブルと確定数のみ**。テーブル外に相性評価・有利不利・推奨・「抜群です」等の散文を **書かない** (CLAUDE.md「事実のみ返す」「タイプ相性は最も間違える箇所」)。ユーザーが解釈を明示的に求めた場合のみ、type-chart の実出力倍率に限って答える。
...
| `Immune (0 damage)` 平文 / または JSON `"ko": "immune"` | 「タイプ相性または特性により無効（ダメージ0）です」と案内（**immune 時は `--format json` 指定でも平文を返すので、JSON parse 前に "Immune" prefix を判定する**） |

## 対応している修飾子

### 攻撃側特性
ちからもち, ヨガパワー, はりきり, サンパワー, テクニシャン, てつのこぶし, がんじょうあご, メガランチャー, すてみ, すなのちから, すいほう, もらいび, てきおうりょく, フェアリースキン, スカイスキン, フリーズスキン, エレキスキン, こだいかっせい, クォークチャージ, トランジスタ, りゅうのあぎと, はがねのせいしん, ちからずく, アナライズ, いろめがね, そうだいしょう, かたいツメ, きれあじ, ひひいろのこどう, ハドロンエンジン, へんげんじざい, リベロ
